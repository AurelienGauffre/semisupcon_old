...
from semilearn.core.hooks import Hook, get_priority, CheckpointHook, TimerHook, LoggingHook, DistSamplerSeedHook, \
    ParamUpdateHook, EvaluationHook, EMAHook, WANDBHook, AimHook, YAMLSAVE_Hook
...
class AlgorithmBase:
    ...
    def set_model(self):
        ...
        model = self.net_builder(num_classes=self.num_classes, pretrained=self.args.use_pretrain,
                                 pretrained_path=self.args.pretrain_path,  args=self.args) #droprate=self.args.droprate résoud le probleme, bug de la lib ?
        return model
    ...
    def set_hooks(self):
        ...
        self.register_hook(YAMLSAVE_Hook(), None, "LOWEST")
